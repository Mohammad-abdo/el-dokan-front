...
import { Wallet, DollarSign, TrendingUp, Package, ArrowDownCircle, Truck } from 'lucide-react';
...
export default function DriverWallet() {
  ...
  const fetchWalletData = async () => {
    try {
      const [dashboardRes, earningsRes] = await Promise.all([
        api.get('/driver/dashboard').catch(() => ({ data: { data: {} } })),
        api.get('/driver/earnings').catch(() => ({ data: { data: {} } })),
      ]);
      ...
      setWallet({
        ...
        total_deliveries: dashboard.total_deliveries || 0,
      });

      // Fetch transactions if endpoint exists
      try {
        const transactionsRes = await api.get('/driver/earnings/transactions');
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: wallet.total_deliveries.toLocaleString(),
      icon: Truck,
