...
import { Wallet, DollarSign, TrendingUp, Calendar, ArrowDownCircle, UserCheck } from 'lucide-react';
import { useLanguage } from '@/contexts/LanguageContext';
import { format } from 'date-fns';

export default function RepresentativeWallet() {
  ...
  const fetchWalletData = async () => {
    try {
      const [dashboardRes, earningsRes] = await Promise.all([
        api.get('/representative/dashboard').catch(() => ({ data: { data: {} } })),
        api.get('/representative/earnings').catch(() => ({ data: { data: {} } })),
      ]);
      ...
      setWallet({
        balance: earnings.total_earnings || dashboard.total_earnings || 0,
        total_earnings: earnings.total_earnings || dashboard.total_earnings || 0,
      ...
      try {
        const transactionsRes = await api.get('/representative/earnings/transactions');
  ...
  const statCards = [
    ...
    {
      title: language === 'ar' ? 'أرباح قيد الانتظار' : 'Pending Earnings',
    ...
    },
    {
      ...
      value: wallet.total_visits.toLocaleString(),
      icon: Calendar,
