  ...
  const handleDelete = async (id) => {
    showConfirm(
      ...
      () => {
        try {
          setDeletingId(id);
          setStatusMessage("Lifestyle question deleted successfully.");
        } catch (error) {
          const message =
            error instanceof Error
              ? error.message
              : "Failed to delete lifestyle question. Please try again.";
    ...
    <section className="space-y-6">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div>
            <h1 className="text-3xl font-semibold text-foreground">
              Lifestyle Questions Management
            </h1>
            <p className="text-sm text-muted-foreground">
              Create and manage lifestyle assessment questions.
            </p>
          </div>
          <Link
            to="/dashboard/admin/lifestyle/create"
        ...
        {isLoading ? (
          ...
          <div className="space-y-3 rounded-3xl border border-destructive/40 bg-destructive/20 p-6 text-destructive-foreground backdrop-blur">
            <p>Failed to load lifestyle questions. Please refresh the page.</p>
            <button
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur">
            <div className="p-6">
              <p className="text-muted-foreground">
                {questions.length === 0
                  ? "No lifestyle questions found. Create one to get started."
