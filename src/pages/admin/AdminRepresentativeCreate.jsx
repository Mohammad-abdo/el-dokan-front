  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        await api.post('/admin/representatives', payload);
      }

      navigate('/admin/representatives');
    } catch (error) {
      console.error('Error saving representative:', error);
      showToast.error(error.response?.data?.message || 'Failed to save representative');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">
              {isEdit ? (language === 'ar' ? 'تعديل المندوب' : 'Edit Representative') : (language === 'ar' ? 'إنشاء مندوب جديد' : 'Create New Representative')}
            </h1>
          </div>
        </div>
      </motion.div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                required
                placeholder={language === 'ar' ? 'مثال: القاهرة - مصر الجديدة' : 'e.g., Cairo - New Cairo'}
              />
            </div>
