...
const driverMenuItems = [
  { icon: LayoutDashboard, label: "Dashboard", labelAr: "لوحة التحكم", path: "/driver/dashboard" },
  { icon: Package, label: "Available Orders", labelAr: "طلبات متاحة", path: "/driver/available-orders" },
  { icon: Truck, label: "Deliveries", labelAr: "التوصيلات", path: "/driver/deliveries" },
  { icon: Wallet, label: "Wallet", labelAr: "المحفظة", path: "/driver/wallet" },
  { icon: Settings, label: "Settings", labelAr: "الإعدادات", path: "/driver/settings" },
];
    ...
    <div className="flex flex-col h-full" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="h-16 flex items-center justify-center border-b border flex-shrink-0">
        ...
        >
          ...
          >
            {isRTL ? "بوابة السائق" : "Driver Portal"}
          </span>
        </motion.div>
        {onClose && (
          <motion.button
      ...
      <div className="p-4 border-b border">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium text-sm truncate">{user?.username || "Driver"}</p>
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-1">
        {driverMenuItems.map((item, index) => {
          ...
          const isActive = location.pathname === item.path || (item.path !== "/driver/dashboard" && location.pathname.startsWith(item.path));
