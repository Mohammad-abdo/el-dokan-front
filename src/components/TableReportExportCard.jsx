...
export function TableReportExportCard({ reportKey, data = [], columns = [] }) {
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="rounded-full"
              />
              <span className="text-sm">{language === 'ar' ? 'سجل واحد (محدد)' : 'One record (selected)'}</span>
            </label>
          </div>
          {scope === 'one' && (
            <div className="mt-3">
              <label className="text-xs text-muted-foreground block mb-1">
                {language === 'ar' ? 'اختر الطبيب / المتجر / الشركة / السائق / المندوب:' : 'Select record:'}
              </label>
              <select
                value={selectedId}
          ...
          <div className="space-y-2">
            <label className="flex items-start gap-2 cursor-pointer text-sm">
              <input
                ...
                className="mt-0.5"
              />
              <span>{language === 'ar' ? 'جدول فقط — نفس أعمدة وصفوف الجدول (يفتح في Excel)' : 'Table only — same columns and rows (opens in Excel)'}</span>
            </label>
            <label className="flex items-start gap-2 cursor-pointer text-sm">
              <input
                type="radio"
                ...
                className="mt-0.5"
              />
              <span>{language === 'ar' ? 'تقرير كامل — جدول + تاريخ التقرير والفلاتر' : 'Full report — table + date and filters'}</span>
            </label>
            {scope === 'one' && (
              <label className="flex items-start gap-2 cursor-pointer text-sm">
                <input
                  ...
                  className="mt-0.5"
                />
                <span>{language === 'ar' ? 'تقرير مفصل — كل بيانات السجل من صفحة التفاصيل' : 'Detailed report — all record data from detail page'}</span>
              </label>
            )}
          </div>
        </div>

        {/* الأعمدة (عندما ليس تقرير مفصل) */}
        {!isDetailed && (
          <div>
            <p className="text-sm font-medium flex items-center gap-1.5 mb-2">
              <ListChecks className="h-4 w-4" />
              {language === 'ar' ? 'ما الذي سيتم تصديره؟' : 'What to export?'}
            </p>
            <p className="text-xs text-muted-foreground mb-2">{language === 'ar' ? 'الأعمدة في الملف:' : 'Columns in file:'}</p>
            <div className="max-h-28 overflow-y-auto rounded-md border bg-muted/30 p-3 space-y-1.5">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              {language === 'ar' ? 'عدد الصفوف:' : 'Rows:'} <strong>{scope === 'one' ? (selectedId ? 1 : 0) : rowCount}</strong>
            </p>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-4 pt-2 border-t">
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer text-sm">
              <input type="radio" name={formatName} checked={exportFormat === 'csv'} onChange={() => setExportFormat('csv')} className="rounded-full" />
              <FileDown className="h-4 w-4" /><span>CSV</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer text-sm">
              <input type="radio" name={formatName} checked={exportFormat === 'excel'} onChange={() => setExportFormat('excel')} className="rounded-full" />
              <FileSpreadsheet className="h-4 w-4 text-green-600" /><span>Excel</span>
            </label>
          </div>
          <Button
          ...
          >
            ...
            {loadingExport ? (language === 'ar' ? 'جاري الاستخراج...' : 'Exporting...') : (language === 'ar' ? 'استخراج التقرير' : 'Export report')}
          </Button>
        </div>
      </div>
    </section>
